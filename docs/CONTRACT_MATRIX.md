...
| POST `/api/auth/refresh` | Reads `refreshToken` cookie only; returns `{ accessToken }`. | Accept refresh token from body (and cookie fallback) for compatibility with mobile clients | Body token unsupported; request validator absent. |
| GET `/api/weather/forecast` | Proxies OpenWeather forecast when API key exists; mock returns `{lat,lng,unit,forecast:[{date,temperatureC,description}]}`. | Stable forecast array structure, normalized date strings, location metadata | Live path returns raw OpenWeather payload (list of 3-hour entries) without normalization; mock/live shapes diverge. |
| GET `/api/faqs` | Not implemented. | id, question, answer, order fields in stable array | Endpoint missing; add stub or DB-backed implementation. |
| POST `/api/upload/itinerary-thumbnail` | Not implemented. | file/URL payload, validation, returned thumbnail URL | Missing route/controller/storage wiring; needs stub with safe placeholder URL if storage pending. |
| GET `/api/users/me/activity-logs` | Returns paginated activity logs scoped to the authenticated user with optional type/date filters; admin `/activity-logs` remains for full access. | scoped activity entries for authenticated user, pagination meta | Self-scope added; keep admin route limited to admins. |
| GET `/api/users/me/stats` | Returns booking stats for the authenticated user with trend/distribution cards filtered by ownership/collaboration. | user profile snippet, cards (total/pending/active/completed), trend labels, status/type distributions | Uses same query param `year` as admin stats; limited to the caller's bookings. |
