...
| GET `/api/users/me/activity-logs` | Not implemented; activity logs are admin-only. | scoped activity entries for authenticated user, pagination meta | Requires self-scope route/guard; current query DTO allows admin actorId filter only. |
